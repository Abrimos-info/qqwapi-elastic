...
const query_definitions = {
  ...
  city: {
    ...
    fields: [
      "area.id.keyword",
      "area.name.keyword",
      "parties.buyer.address.locality",
    ],
  },
  // apiFilterName: "name",
  // apiFieldNames:["name"],
  ...
  classification: {
    ...
    change_qdp: {
      //Modify query definition for another field based on the value of this field
      country: {
        value: "city",
        context: "must",
        type: "match",
  ...
  buyer_name: {
    ...
    fields: [
      ...
      "parties.buyer.id",
      "parties.buyer.memberOf.id",
      "parties.buyer.memberOf.initials",
    ],
    launder: true,
  },
  ...
  contact_point_name: {
    ...
    type: "multi_match",
    fields: [
      "parties.buyer.contactPoint.name",
      "parties.buyer.contactPoint.id",
    ],
  },
        ...
        aggs: {
          ...
          monto_total_sobrecosto: {
            scripted_metric: {
              ...
              map_script:
                "if (doc['contracts.items.unit.value.amountOverpriceMxIMSS'].size()>0) { def o = doc['contracts.items.unit.value.amountOverpriceMxIMSS'].value; state.sobrecostos.add( o > 0 ? o : 0) }",
              combine_script:
                "double sobrecosto = 0; for (t in state.sobrecostos) { sobrecosto += t } return sobrecosto",
              reduce_script:
                "double sobrecosto = 0; for (a in states) { sobrecosto += a } return sobrecosto",
            },
          },
          sobrecosto: {
            scripted_metric: {
              init_script: "state.sobrecostos = []",
              map_script:
                "if (doc['contracts.items.unit.value.percentageOverpriceMxIMSS'].size()>0) { def o = doc['contracts.items.unit.value.percentageOverpriceMxIMSS'].value; state.sobrecostos.add( o > 0 ? o : 0) }",
              combine_script:
                "double sobrecosto = 0; for (t in state.sobrecostos) { sobrecosto += t } return sobrecosto/state.sobrecostos.length",
              reduce_script:
                "double sobrecosto = 0; for (a in states) { sobrecosto += a } return sobrecosto / states.length",
            },
          },
          cantidad_perdida: {
            sum: {
      ...
      if (qdp) {
        ...
        if (qdp.context == "skip") {
          ...
          if (
            ...
            if (qdp.field) {
              ...
              values.map((value, index) => {
                ...
                function_score[qdp.type].functions.push({
                  weight: 100000000000000 + values.length - index,
                  filter: { match: { [qdp.field]: value } },
                });
              });
            }
            ...
            values.map((value, index) => {
              ...
              function_score["function_score"].functions.push({
                field_value_factor: {
                  field: value,
                  factor: 1 + (values.length - index) / 10,
                  missing: 0,
                },
              });
            });
          ...
          if (qdp.type == "range-lte") {
            body.query.bool[qdp.context].push({
              range: { [qdp.field]: { lte: params[param] } },
            });
          }
          if (qdp.type == "range-gte") {
            body.query.bool[qdp.context].push({
              range: { [qdp.field]: { gte: params[param] } },
            });
          } else {
  ...
  if (!params.query.embed) {
    ...
    if (edis) {
      ...
      for (e in edis) {
        ...
        results.hits.hits.forEach((result) => {
          ...
          if (edi.condition) {
            if (debug) {
              console.log(
                "embed condition",
                edi,
                result._source[edi.condition.field] ==
                  edi.condition.negativeValue,
              );
            }
            if (
              result._source[edi.condition.field] == edi.condition.negativeValue
            ) {
              resultValid = false;
            }
          }
          ...
          if (resultValid) {
            if (edi.id) {
              ...
              searchDocument.body.query.bool.should.push({
                match_phrase: { [foreignKeyword]: result._source[edi.id] },
              });
            }
            ...
            function addFieldValue(type, field) {
              ...
              if (fieldValue) {
                searchDocument.body.query.bool[type].push({
                  match_phrase: { [field]: fieldValue },
                });
              }
            }
            ...
            if (edi.aggs) {
              for (a in edi.aggs) {
                searchDocument.body.aggs[edi.aggs[a].location] =
                  edi.aggs[a].definition;
              }
            }
          }
        });
        ...
        if (debug) {
          console.log(
            "embed searchDocument body",
            edi.index,
            JSON.stringify(searchDocument.body),
          );
        }

        try {
          if (
            searchDocument.body.query.bool.should.length == 0 &&
            searchDocument.body.query.bool.must.length == 0 &&
            edi.size != 0
          ) {
            console.log(
              "Embed",
              "Empty search document in index: ",
              index,
              "location:",
              edi.location,
              edi.id,
          ...
          if (embedResult.body.aggregations) {
            ...
            results.hits.hits[0]._source[edi.location + "_summaries"] =
              formatSummary(embedResult.body.aggregations, debug);
          }

          for (r in results.hits.hits) {
            for (h in embedResult.body.hits.hits) {
              if (edi.foreign_key) {
                let foreign_key_value = fieldPathExists(
                  edi.foreign_key,
                  embedResult.body.hits.hits[h]._source,
                );
                if (debug) {
                ...
                if (
                  foreign_key_value[0] &&
                  foreign_key_value[0] == results.hits.hits[r]._source[edi.id]
                ) {
                  ...
                  if (edi.add) {
                    embedResult.body.hits.hits[h]._source = Object.assign(
                      {},
                      embedResult.body.hits.hits[h]._source,
                      edi.add,
                    );
                  }
                  embedResult.body.hits.hits[h]._source.type = get_db_type(
                    embedResult.body.hits.hits[h]._index,
                  );
                  results.hits.hits[r]._source[edi.location].push(
                    embedResult.body.hits.hits[h]._source,
                  );
                ...
                if (
                  edi.dontRepeatSelf &&
                  results.hits.hits[r]._source.id ==
                    embedResult.body.hits.hits[h]._source.id
                ) {
...
async function search(index, params, prefix, debug) {
  ...
  if (params.query.months_summary) {
    searchDocument.body.aggs = Object.assign(
      {},
      searchDocument.body.aggs,
      contracts_summary_month,
    );
  }
  ...
  if (
    parseInt(searchDocument.body.from) + parseInt(searchDocument.body.size) <
    10000
  ) {
    if (debug) {
    ...
    console.error(
      "Search size is bigger than 10000",
      "bodysize",
      searchDocument.body.size,
      "bodyfrom",
      searchDocument.body.from,
      "sum",
      parseInt(searchDocument.body.from) + parseInt(searchDocument.body.size),
    );
    return {
      error:
        "Search size is bigger than 10000. Elasticsearch does not allow it.",
    };
  }
}
...
function get_db_type(index) {
  ...
  // Remove qqw_ prefix if present
  if (index.indexOf("qqw_") == 0) {
  ...
  // If no underscores, return the index name as-is
...
function prepareOutput(body, context, debug) {
  ...
  //Unable to parse
    ...
    console.error(
      "prepareOutput error",
      bodyhits ? bodyhits.error : "empty bodyhits",
      body ? body.error : "",
      stack_trace,
    );
  ...
  if (
    ...
    errorText.meta &&
    errorText.meta.body &&
    errorText.meta.body.error &&
    errorText.meta.body.error.type
  ) {
      ...
      if (typeof aggs[key] == "number") {
        ...
        for (bucket_index in aggs[key].buckets) {
          ...
          if (Object.keys(bucket).length > 2) {
            ...
            if (bucket.key && bucket.key.endsWith("qqw_")) {
              bucket.key = bucket.key.substring(0, bucket.key.length - 4);
            }
  ...
  for (var i = 0; i < fieldPath.length; i++) {
    ...
    if (isArray(tempObj[fieldPath[i]])) {
      ...
      if (i == fieldPath.length - 1) {
        // Estamos chequeando si existe el array, no su valor
        fieldValues.push(tempObj[fieldPath[i]]);
      } else if (tempObj[fieldPath[i]].length > 0) {
        // Iteramos sobre el array de campos
        tempObj[fieldPath[i]].map((tempItem) => {
          var results = fieldPathExists(
            fieldPath.slice(i + 1, fieldPath.length).join("."),
            tempItem,
          );
      ...
      isString(tempObj[fieldPath[i]]) ||
      isNumeric(tempObj[fieldPath[i]])
    ) {
      // Value of the field is a string or number
      if (i < fieldPath.length - 1) {
        // Arrived at a string or number while end of path has not been reached
        return fieldValues;
      }
      if (
        tempObj[fieldPath[i]] == "" ||
        tempObj[fieldPath[i]] == "---" ||
        tempObj[fieldPath[i]] == "null"
      ) {
        // Arrived at empty string, '---' or 'null'
        return fieldValues;
      }
      fieldValues.push(tempObj[fieldPath[i]]);
      return fieldValues;
      ...
      if (i < fieldPath.length - 1) {
        // Arrived at a date while end of path has not been reached
        return fieldValues;
      }
      fieldValues.push(tempObj[fieldPath[i]].toISOString());
      return fieldValues;
    } else if (
      tempObj.hasOwnProperty(fieldPath[i]) &&
      !isEmpty(tempObj[fieldPath[i]])
    ) {
