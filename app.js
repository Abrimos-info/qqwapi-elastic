...
async function createServer() {
  ...
  app.use((request, response, next) => {
    // CORS headers are set by nginx to avoid duplication
  ...
  app.use((err, req, res, next) => {
    console.error(
      "Internal error",
      req.route.path,
      err.meta ? err.meta.meta.request.params : "",
      err.meta ? (err.meta.body ? err.meta.body.error : "") : "",
      err,
    );
  ...
  const l = server.listen(port, () => {
    process.stdout.write(
      `Listening on http://localhost:${l.address().port}/\n`,
    );
  });
}
